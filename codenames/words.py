...
def test_regexps():
    ...
    assert regex_non_word.sub("", "asdf ASDF a8sdf a-sdf a_b") == "asdf ASDF  a-sdf "
...
def filter_non_german(mydict, dictdict):
    return csort({key: value for key, value in mydict.items() if key in dictdict})


def remove_top_n(mydict, dictdict, n=100):
    to_remove = list(dictdict.keys())[:n]
    return csort({key: value for key, value in mydict.items() if key not in to_remove})
...
def remove_similar(dct):
    ...
    for word in words:
        for word2 in words:
            if word[:5] in word2 and len(word) < len(word2) and not word == word2 and word2 in words2:
                words2.remove(word2)
    ...
    freqs = remove_similar(
        top_n(
            freq_ratio(
                nouns_only(
                    remove_top_n(
                        filter_non_german(freqs, freqdict),
                        freqdict,
                        300
                    )
                ),
